...
the optimization routine sometimes briefly settles into a local minumum but is 
...
The EI criterion is fundamentally based on the improvement criterion {\color{red}cite} which evaluates how possible it may be to encounter new minima at a given location based on the predictive surrogate model.
%
...
\end{equation}
%
By considering the expectation of $\ix$, candidate locations are not only rewarded for having a low predictive mean, but the $\Eix$ also rewards poorly explored locations due to the high uncertainty of $\ix$ in these places.
%
Notice that by definition the $\ix$ function is always non-negative, however the GP posterior predictive $\Eix$ is a strictly positive criterion.
%
Considering the MCMC inferential setting of our GP surrogate model, the EI criterion can be quickly computed by using posterior predictive $\ix$ samples at given candidate locations to empirically approximate the $\Eix$ calculation.
...
Since preconvergence ELAI convergence criterion evaluations do not neccisarily share consistency amoung all observed values, a weighted series perspective was chosen to follow the moving average of the most recent ELAI observations while still smoothing with some memory of older evaluations.
...
%
It is interesting to note that for the example series used in Figure~(\ref{bestL}), the optimal $\hat \lambda$ is found to be about 0.4, which obviously falls outside of the standard values for $\lambda$ mentioned above.
%would begin with starts
The typical use of EWMA in SPC begins with the premise of a relatively stable series and attempts to identify new out-of-control observations which would indicate some change in the data generating process.
%
The use of EWMA to identify convergence is markedly different from the typical SPC usage. 
%
For identifying convergence the series begins in an out-of-control state and we wish to identify when the series has fallen into control (i.e. convergence).
%
...
Often the optimal $\hat\lambda$, for identifying convergence, may fall above the recommended upper limit for $\lambda$. 
%
Thus, in this context it is useful for to borrow some of the machinery used in SPC, but the ultimate analysis of the data should not explicitly be considered SPC, as the perspective of the data does not fully align with that of SPC.
...
The Rosenbrock function \cite{rosePaper} was an early test probelm in the optimizaiton literature. It combines a narrow, flat parabolic valley with steep walls, and thus it can be difficult for gradient-based methods. It is generalizable to higher dimensions, but we use the two-dimensional version here.
	\begin{eqnarray}
