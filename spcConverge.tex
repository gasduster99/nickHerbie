...
%
\item Gaussian process surrogate modeling has become a standard tool for analysis of computer emulation experinents \cite{sacksDesign, santnerBook} as well as many 
%
\item other applications in which an a'priori assumption of smoothness is useful \cite{abrahamsenBook, steinBook}.
%
\item For the sake of optimization, the assertion of a smooth objective function, $f$, serves as the basis for most efficient searching algorithms.%, as is explicatly the case with GP.
%
\item In the case of GP surrogate modeling, the Gaussian Process theory can explicatly express this desire for smoothness through the a'prior choice of a covariance function with the desired smoothness properties.
%
\item Although smoothness is key to search, and 
%  
\item For the sake of optimization practically all opt we only have a hope of efficently searching $f$ if we assume that $f$ provides a reasonably smooth, otherwise optimization by any method is practically infeasible.
%
