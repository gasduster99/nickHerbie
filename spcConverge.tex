...
%
However for complex simluators, the response surface may be ill-behaved and optimization
...
{\color{red}XX}, wherein contamination in the groundwater near the
...
Because Expected Improvement is non-negative but decreasingly small,
...
which the simple threshold stopping rule can accuratly identify convergence.
%as this series
...
%
The right-most panel (the Rastigin test function) 
...
%
In the spirt of obtaining a robust solution through the use of the CLT, as applied to the distribution of the EI, it is important to carefully consider properties of the improvement distribution.
%
Firstly, the improvement values are non-negative but decreasingly small, thus the improvement distribution is often strongly right skewed.
%
...
%Additionally, since the I(x) criterion is naturally bounded at 0, an unfettered normal distribution will always struggle to model the EI criterion since the normal distribution will never respect itsboundary conditions. 
Secondly, since the improvement distribution is non-negative, the EI should also be bounded at 0.
%
...
%
However due to the MCMC sample-based implimentation of the Gaussian Process, and the desire for a large number of samples from the improvement distribution, it is not uncommon to obtain at least one sample that, for computaional purposes, is indistinguishable from 0.
%
...
%
Thus rather than simply taking the log of the improvement samples, to determine the more robust statement that $\mathbb{E}\left[~\log\text{I}~\right] \sim N\left(\mu, \frac{\sigma^2}{n}\right)$, it is computationally useful to consider the following {\color{red}theoretically equivalent} perspective.   
%
...
%
Furthermore, if $m$ and $v$ are, respectively, the mean and variance of a log-normal sample, then the mean, $\omega$, and variance, $\nu$, of the associated normal distribution are given by the following relation {\color{red}cite}.
%
\begin{eqnarray}
...
\end{eqnarray}
%
Using this relation we do not need to transform any of the improvement samples; we can instead jump straight to the mean of the samples that would have resulted from log transforming the improvement samples.
%
That is to say, by taking the mean of the unaltered improvement values and using relation (\ref{lnRelate}) we immediately get empirical $\E{\log \text{I}}$ values without actually taking the log of the improvement samples.
%rather than
Considering that the log improvement values would no longer be bounded at 0, the log improvement distribution will have reduced right skewed, and thus the distribution of the log improvement provides robust asymptotics for the $\E{\log\text{I}}$ distribution, even as convergence approaches. {\color{red} ELAI}
