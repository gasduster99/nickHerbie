...
Arising naturally from the GP predictive distribution \cite{gBook, EIPPD}, the maximum Expected Improvement (EI) criterion has shown to be a valuable criterion for guiding the exploration of the objective function \cite{taddyOpt, jonesEIOpt}; furthermore the EI shows promise for use as a convergence criterion {\color{red}cite}.
...
However, the basic idea behind the use of EI as a convergence criterion is that convergence should occur when the surrogate model produces low expectations for discovering new optimum; that is to say, globally small EI values should be associated with convergence of the algorithm.
%claim that there exists 
Thus an obvious stopping rule first defines some lower EI threshold, then claims convergence upon the first instance of the maximum EI value falling below this threshold {\color{red} example \cite{windExample}, cite}.    
...
This use of EI as a convergence criterion in analogous to the standard approach in the numerical optimization literature of setting a threshold for some value that is monitored and declaring convergence when the threshold is obtained (e.g., the vanishing step sizes of a Newton-Raphson algorithm).
%
...
%
The surrogate model serves as a tool for using function evaluations, to infer the expected behavior of the objective function and thus determine where further optima may exist with minimal evaluation the objective function itself.
%
Surrogate modeling is therefore useful for optimizing large computer simulations experiments, where each function evaluation may consume considerable computational resources.
%
...
%
As a result GP surrogate models have become a standard tool for analysis of computer emulation experiments \cite{sacksDesign, santnerBook}.
...
However for the sake of providing a flexible surrogate model, it is desirable to have the ability to loosen these restrictions in cases when $f$ may have inherently sharp boundaries, or numerical simulators have variable stability in portions of the domain. 
%
Gramacy and Lee \cite{gpJasa} use the idea of allowing subpopulations of flexibility via a treed partitioning of the domain, fitting stationary GP surfaces to separately stationary portions of $f$.
%
The domain is recursively sub-partitioned and separate hierarchically linked GP models are fit within each sub-partition.
%
The partitioning scheme is fit via a reversible jump MCMC algorithm, jumping between models with differing partitioning schemes.
%
By partitioning the domain in this way it allows parsimonious surrogate models in simple objective function cases and quite flexible surrogate models when the the objective function display's complex behavior. 
%
For further explanation of partitioned Gaussian process models as well as notes on implementing such models in R, see the R package \verb tgp  \cite{tgp, tgp2}.  Because many of the objective functions of interest are not well modeled by a stationary GP, we use treed GPs as our surrogate models in this paper.
...
%
The improvement criterion is strictly positive but decreasingly small, thus the improvement distribution is often strongly right skewed, and the EI is often far from normal.
%
...
%
This construction allows for more consistent and accurate use of the fundamental theory on which our SPC perspective requires.
...
%
The EWMA control chart is based upon Shewhart's original notion of control, however the EWMA control chart views the typical repeated sampling process in the context of moving average smoothing of series data. %, rather than assuming a constant long run average, as in the $\bar x$-chart.    
%Principally provide particularly robust solutions to be expected for convergence in this setting.
Since pre-convergence ELAI convergence criterion evaluations do not necessarily share distributional consistency among all observed values, a weighted series perspective was chosen to follow the moving average of the most recent ELAI observations while still smoothing with some memory of older evaluations.
...
Due to the decreased stability of the series, in this context, the optimal forecasting $\hat\lambda$ may often fall above the recommended upper limit for $\lambda$, in-order to more accurately follow the more active moving averages inherent to the unstable pre-convergence series. 
%
Thus, in this context it is useful for to borrow the EWMA machinery often used in SPC, but the ultimate analysis of the data should not explicitly be considered SPC, as the perspective of the data does not fully align with that of SPC.

%
%

%
For identifying convergence it is of primary importance to define the control limits on the EWMA statistic in this setting.
%
As in the simplified $\bar x$-chart, defining the control limits in the EWMA setting amounts to considering an interval on the sampling distribution of interest.
...
%
Furthermore by choose a confidence level through choice of a constant $c$, the control limits based on this sampling distribution are seen in Eq. (\ref{EWMACL}).% follow on the next page.
...
%
The realization of such a controlled region of the series defines the notion of consistency which is part of what allows for the identification of convergence here. 
...
%
The size of the control window, $w$, may vary from problem to problem, based on the difficulty of optimization in each case; ultimatly the choice of $w$ is left as a tuning parameter of the system.
%
...
However, just as in sample size calculations, the the choice of an optimal $w$ must consider the cost of premature identification of convergence (i.e. poor inference), associated with underestimating $w$, against the cost of computing diminishingly necessary objective function evaluations (i.e. the cost of sampling), associated with overestimating $w$.
%
To provide a default choice of $w$ is somewhat arbitrary without careful analysis of the particulars of the objective function behavior and the costs of each successive objective function evalutation, however as a recommendation for starting this analysis one may consider the rule $w\ge30$ representing the prior assertion that premature identification of convergence is a worse error than computing extraneous objective function evaluations. 
...
%
Similar effects may be observed for highly multimodal objective functions, as the regular discovery of new modes will increase the variance of the ELAI criterion, and disguise any decreasing mean value, amoung the noise inherent to the search of such functions.
...
%
By contrast strongly unimodal functions, will enjoy a relative fast decrease in the ELAI criterion in the presence of relatively small variability.
%
This relatively high signal-to-noise ratio makes for easier identification of convergence, and thus requires a smaller choice of $w$ to notice a move of the ELAI criterion into convergence.
%
However if $w$ is chosen to be too small, the algorithm may be over eager to claim convergence and the reccomendation of $w\ge30$ is particularly apt here to guard against false identification of convergence.   
...
%
The decreased value of $\hat\lambda$ reflectes the increased use of $\lambda$ as a smoothing parameter, relative to rosebrock.
%
...
%
In this application $w=40$, as chosen by experimentation.
%
...
%
The left panel of Figure~(\ref{fig:rastrigin}) shows the convergence chart (left) and the optimization progress of the algorithm (right) after 95 iterations of optimization.
%
...
%
Furthermore notice that optimization progress in Figure~(\ref{fig:rastrigin}, right) demonstrates that convergence in this case does indeed represent the identification of the theoretical minimum of the the function, as indicated by the dashed horizontal line at the theoretical minimum. 
...
%
This is done for the sake of developing an intuition for tuning the EWMA convergence chart parameters and to ensure that our methods correspond to the identification of an optimal value.
...
%
To demonstrate the use of the EWMA convergence chart in a practical optimization problem, we consider the Lockwood pump-and-treat problem originally presented by Matott et al. \cite{lockCite}.

%
%

%
The Lockwood pump-and-treat case study considers a contaminated {\color{red}blah-blah} site north-east of Billings, Mountana along the Yellowstone River.%of contaiminted groundwater tha pumping site for chlorinated solvents located north-east of Billings, Montana, along the Yellowstone River.
%
If left untreated, the contaminated groundwater, from this site, will contaminate the Yellowstone river with chlorinated solvents. 
...
Inorder to control this contaminated groundwater, 6 pumps are used to redirect groundwater away from the river.
%
...
%
Pumping each of these six wells at different rates results in various complicated groundwater behahviors, that require the evaluation of a numerical simulation of the system to predict.
%
...
%
The objective function, $f(\bm{x})$, can be expressed as the sum of the pumping rates (a quanity proportional to the cost of running the pumps), with large penalties associated with contamination of the river. % by each plume, $c_A(\bm{x})$, $c_B(\bm{x})$. %$c_A(\bm{x})$ and $c_B(\bm{x})$ indicating that the  
        %, is the cost of operating the pumps, in USD; additionally, $f$ heavily penalizes solutions that contaminate the river.
\begin{equation}
f(\bm{x}) = \sum_{i=1}^6 x_i +  2 C(\bm{x}) + 20000 \oner_{ C(\bm{x})>0 }  
\end{equation}
%with simple search boundaries, \mbox{$0\le x_i\le20,000$} set for each pumping rate.
Here $C(\bm{x})$ indicates the amount of contamination of the river, and $\bm{x}$ represents the pumping rates for each of the six wells.
%, \mbox{$\left[x_1, ..., ~x_6\right] = \left[Q_{A1}, ~Q_{A2}, ~Q_{B1}, ~Q_{B2}, ~Q_{B3}, ~Q_{B4}\right]$}.
Each $x_i$ is bounded on the large interval, \mbox{$0\le x_i\le20,000$}.
%
The full problem defines a six-dimensional optimization problem, to determine the optimal rate at which to pump each well.
%
Since running the numerical simulation of the system is 
