...
{\color{red} *Everybody uses GP, bunch of cites}\\
%
Gaussian process surrogate modeling has become a standard tool for analysis of computer emulation experinents \cite{sacksDesign, santnerBook} as well as many other applications in which an a'priori assumption of smoothness is useful \cite{abrahamsenBook, steinBook}.
%
For the sake of optimization, the assertion of a smooth objective function, $f$, serves as the basis for most efficient searching algorithms.%, as is explicatly the case with GP.
In the case of GP surrogate modeling, the Gaussian Process theory can explicatly express this desire for smoothness through the a'prior choice of a covariance function with the desired smoothness properties.
%
Although smoothness is key to search, and 
% provides a reasonably smooth mapping fromt he domai 
%For the sake of optimization practically all opt we only have a hope of efficently searching $f$ if we assume that $f$ provides a reasonably smooth, otherwise optimization by any method is practically infeasible.
%
However it is not 
% If we have any hope of finding optima of a function, $f$, we impose the condition that $f$ provides a reasonably smooth mapping for relating points in the domain, $\bm{x}$, to response values, $z(\bm{x})$.
%
\cite{gpJasa, tgp, tgp2}

{\color{red} *Expand Treed Partitioning}

For additional modeling details, including loosening the assumption of global stationarity, and details about implementing GP models in the context of numerical optimization see the R package \verb tgp ~as well as the associated vignettes \cite{tgp, tgp2}.
...
%Considering that the log improvement has reduced right skew, for the sake of improved asymptotics of the; of the $\E{\log\text{I}}$ distribution
...
%
The EWMA control chart is based upon Shewhart's original notion of control, however the EWMA control chart veiws the repeated sampling process in the context of moving average smoothing of series data. %, rather than assuming a constant long run average, as in the $\bar x$-chart.    
...
%
Large values of $\lambda$ assign more weight to recent obsevations in the series, allowing for a more flexible fit for unstable series.
%
However, the choice of a large $\lambda$ may overfit the $Z_i$ to noise in the $Y_i$.
%
It is thus desireable to choose to the smallest $\lambda$ which still provides good forcasts of future
%
%
...
%
Box et al. \cite[p. 87]{boxBook} explains how to choose an optimal value for $\lambda$ by choosing the $\hat\lambda$ which minimizes the sum of squared forcasting deviations ($S_\lambda$) for each new observation.
%curvature
...
%
As a result, ELAI values for tracking convergence are inherintly less stable than typical SPC applications of EWMA.
...
%
Thus, in this context it is useful for to borrow some of the machinery used in SPC, but the ultimate analysis of the data should not explicatly be considered SPC, as the perspective of the data does not fully align with that of SPC.
...
%
The realization of such a controlled region of the series defines the notion of consitency which is part of what allows for the identification of convergence here. 
...
The final structural feature of the EWMA convergence chart that is necissary for identifying convergence is the so called {\it control window}.
...
% examination
Firstly it serves to dichotomizes the series for evaluating subsets of the $Y_i$ for distibutional consistency.
% convergence
