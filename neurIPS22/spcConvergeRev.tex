...
%designed with the recognition of subtlety shifting means in mind {(\color{red}cite)}.
...
%
%For example, if the objective function is in many dimensions, the choice of $w$ will necessarily increase to allow the surrogate model to gather a proportional amount of information about $f$.  
%
...
%However, j(i.e. the cost of over additional sampling)
...
% We then extrapolate further choices of $w$ based on observed ELAI variance Lockwood pump and Treat based on the observed ELAI variances. 
% 
...
%so as to separate out the increase noise in the ELAI series from  signal.% the increased noise in the ELAI series to a greater degree, as values more by reflects the increased use of $\lambda$ as a smoothing parameter, relative to Rosenbrock.
...
% has shown to be effective and smaller choices of was chosen here 
...
\begin{enumerate}
  ...
  \item Did you discuss any potential negative societal impacts of your work?
    \answerNA{
	Negative societal impacts in the field of optimization relate the particular 
	objective functions being optimized rather than the methods used to 
	determine convergence of the optimization algorithm itself. 
	} 
...
\begin{enumerate}
  ...
  \item Did you report error bars (e.g., with respect to the random seed after running experiments multiple times)?
    \answerNo{
	While we have run replicates, it isn't clear how to measure uncertainty. 
    }
  \item Did you include the total amount of compute and the type of resources used (e.g., type of GPUs, internal cluster, or cloud provider)?
    \answerNo{
	Details about computation time will vary between particular implementations of 
	different Bayesian optimization algorithms. Computation used in the  
	Bayesian optimization algorithm itself is not the point of 
	this paper, and identification of convergence requires a
        relatively small amount of computation time relative to the
...
\begin{enumerate}
  \item If your work uses existing assets, did you cite the creators?
    \answerYes{The Lockwood example uses an existing asset and the original author \cite{lockCite} is cited.}
  \item Did you mention the license of the assets?
    \answerNo{
	We are not aware of a public license for the Lockwood code. The Lockwood case study 
	is merely used here as an example. The details of the Lockwood code are not 
	fundamental to our method of determining convergence.
	}
  \item Did you include any new assets either in the supplemental material or as a URL?
    \answerNo{Example code is given in the supplemental material.}
